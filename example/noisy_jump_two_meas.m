...
function [ok]= t_general_noisy_jump_two_meas(draw_plot, conVar, conFreq)
...
h_fun = @(k,x) direct_h2(k,x,params);
...
z1 = x2_true + v_true + nI.*newNoise;
z2 = x2_true + sigma*randn(1,N);
z = [z1;z2];
...
for k = 1 : N
    rinv(1,1,k) = rinvk; % have a lot of bad measurements
    if (mod(k, 10) == 0)  % have 1/10 good measurements
        rinv(2, 2, k) = rinvk;
   end
end
...
if draw_plot
    ...
    measOne = z(1,:);
    measTwo = z(2,:);
    measOne(measOne > v_max) = v_max;
    measOne(measOne < v_min) = v_min;
    measTwo(measTwo > v_max) = v_max;
    measTwo(measTwo < v_min) = v_min;

    rvecOne = rinv(1,1, :);
    rvecTwo = rinv(2,2, :);
    measOne(rvecOne==0) = v_max + 5; % take non-measurements out
    measTwo(rvecTwo==0) = v_max + 5; % take non-measurements out
  ...
  % % --------------------------------------------------------------------------  
    ...
    plot(t', measOne, 'bd',  'MarkerFaceColor', mcolorOne, 'MarkerSize', msizeOne);
    plot(t', measTwo, 'rd',  'MarkerFaceColor', mcolorTwo, 'MarkerSize', msizeTwo);
...
%    dir = '/Users/aleksand/Dropbox/AnnalsELQP/TStudentSIOPT/TFigures/';
 %   nameBase = 'NoisyJumpCompareTwoMeas';
    
  %  savefig(1:4, '/Users/aleksand/Dropbox/AnnalsELQP/TStudentIEEEtran/TFigures/NoisyJumpCompare');
  %     savefig(1:5, [dir nameBase 'var' num2str(conVar) 'freq' num2str(conFreq*100)]);
